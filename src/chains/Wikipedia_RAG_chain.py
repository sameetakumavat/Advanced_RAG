...
def wikipedia_retriever(input: dict) -> str:
    ...
    docs = resource_service.wikipedia_retriever(question)
    return docs
...
_inputs = RunnableParallel(
    {
        ...
        "context": {"question": lambda x: x.question} | RunnableLambda(wikipedia_retriever),
    }
).with_types(input_type=RagInput)

wikipedia_rag_chain = _inputs | RunnableLambda(get_prompt_template) | chat_model.with_structured_output(CitedAnswer)
